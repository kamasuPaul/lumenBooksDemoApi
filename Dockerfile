...
RUN openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/apache2/certificate/apache.key -out /etc/apache2/certificate/apache.crt -subj "/C=US/ST=Denial/L=Springfield/O=Dis/CN=www.example.com"
# enable ssl
# enable mod_rewrite
RUN a2enmod rewrite
RUN a2enmod ssl
